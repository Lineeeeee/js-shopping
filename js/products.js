      ...
      for(var p of pager.data){
        html +=  `
          <div class="product" id="${p.lid}">
            <a href="product_details.html?lid=${p.lid}"><img src="${p.pic}" alt="${p.title}"/></a>
            <div>
              <p><span class="price">￥${p.price}</span><a href="product_details.html?lid=${p.lid}" class="title" title="${p.title}">${p.title}</a></p>
              <div>
